...
/////VARIABLES LUZ//////
int LightValue = -1; //Luz por defecto, este valor se actualizará una vez lleguen valores del arduino
...
String SpeedValueS = "0"; // String del valor de la velocidad
String v = "";
...
void setup() {
  ...
  cp6.addSlider("Valor velocidad").setPosition(140,240).setSize(220,40).setRange(0,9); 
...
void showHelp(){
  textSize(25);
  fill(255,255,255);
  text("HELP: Control & Monitorización", 380, 50);
...
void ConexionArduino(){
  ...
  //Circulito en rojo con mensaje "Arduino disconnected" si el arduino esta desconectado
     ...
     strokeWeight(0);
     textSize(14);
     text("Arduino desconectado", 55, 575);
     fill(220,20,60);
     circle(40,570,15);
     tab = 1;
  ...
  int umbralTEMP = (int) cp5.getController("UMBRAL temp").getValue(); //Sacamos  constantemente el valor del umbral de temperatura
      ...
      if(LightValue > umbralDay) {
        if(!EnoughLight){
          ...
          //Publicamos en el canal de LED1 el valor de "ON"
           println("Send control 'X' through serial on LED1: "+ "ON");
           port.write('X');
        }
      ...
      //Si el valor de luz es inferior al umbral establecido, se apaga la Luz
        ...
        if(EnoughLight){
           ...
           println("Send control 'X' through serial on LED1: "+ "OFF");
           port.write('X');
        }
      }
      ...
      if(TempValue > umbralTEMP) {
        if(!EnoughTemperature){
         ...
         println("Send control 'Y' through serial on LED2: "+ "ON");
          port.write('Y');
        }
      ...
      //Si la temperatura esta por debajo del umbral establecido, se apaga la LED2  
        ...
        if(EnoughTemperature){
           ...
           EnoughTemperature = false; 
            ...
            println("Send control 'Y' through serial on LED2: "+ "OFFf");
           port.write('Y');
        }
      }
...
void handleButtonEvents(GButton button, GEvent event) {
  ...
  if (button == btn && event == GEvent.CLICKED) {
    contentpwd = pwd1.getPassword();
     writeLog(logFile,"password attempt: "+"'"+ contentpwd+"'");
     println("Password attempt by user: "+"'"+ contentpwd+"'");
     if(passwordValidate.equals( contentpwd)){
       writeLog(logFile, "Contraseña Correcta");
       println("Correct password");
       CorrectPassword = true;
        WrongPassword = false;
     }else{
        writeLog(logFile, "Contraseña Incorrecta");
  ...
  if  (button == btn3 && event == GEvent.CLICKED) {
         if(!motor){
           ...
           motor = true;
           //clockwise=true;
           println("Send control 'M' through serial to Motor: "+ "ON");
           port.write('M');
         }
  }
  ...
  if  (button == btn4 && event == GEvent.CLICKED){
        if(motor == true){
           ...
           motor = false;
             println("Send control 'M' through serial to Motor: "+ "OFF");
           port.write('M');
         }  
  }
   ...
   if  (button == btn5 && event == GEvent.CLICKED) {
         if(LED == true){
           ...
           LED = false;
           println("Send control 'Z' through serial to LED3: "+ "ON");
           port.write('Z');
         } 
         else {
           writeLog(logFile, "LED 3:   ON");
           LED = true;
            println("Send control 'Z' through serial to LED3: "+ "OFF");
           port.write('Z');         
         }
  }
  ...
  if  (button == btn6 && event == GEvent.CLICKED) {
      if(Connected){
        ...
        // println(SpeedValue);
         ...
         writeLog(logFile, "VELOCIDAD AJUSTADA AL nivel: "+msg1);
          println("Send control through serial to MOTOR LEVEL SPEED, level : "+msg1);
         port.write(msg1);
  ...
  if  (button == btn7 && event == GEvent.CLICKED) {
    if(Connected){
      ...
      clockwise = false;
      anticlockwise=true;
       writeLog(logFile, "DIRECCIÓN AJUSTADA: SENTIDO ANTI-HORARIO");
        println("Send control 'L' through serial to MOTOR DIRECTION: "+ "ANTI-CLOCKWISE");
       port.write('L');
    }
  }
  //Para cambiar la dirección del motor a dirección Horario
    ...
    if(Connected){
      ...
      writeLog(logFile, "DIRECCIÓN AJUSTADA: SENTIDO HORARIO");
        println("Send control 'R' through serial to MOTOR DIRECTION: "+ "CLOCKWISE");
       port.write('R');
...
void controlMotor(){
  ...
  textSize(20);
  fill(255,255,255);
  String string = "Nivel de velocidad actual: "+SpeedValueS;
  text(string,140,350);
  
  
  if(SpeedValueS.equals("0")){
    v = "165";
  }
  if(SpeedValueS.equals("1")){
    v = "175";
  }
  if(SpeedValueS.equals("2")){
    v = "185";
  }
  if(SpeedValueS.equals("3")){
    v = "195";
  }
  
  if(SpeedValueS.equals("4")){
    v = "205";
  }
  if(SpeedValueS.equals("5")){
    v = "215";
  }
  if(SpeedValueS.equals("6")){
    v = "225";
  }
  if(SpeedValueS.equals("7")){
    v = "235";
  }
  if(SpeedValueS.equals("8")){
    v = "245";
  }
  if(SpeedValueS.equals("9")){
    v = "255";
  }
  String string2 ="Velocidad del motor: "+ v + " RPM";
  text(string2,140,370);
}
...
void serialEvent(Serial myPort) {
  ...
  if(text != null){
   contentMessage = trim(text);
   lastMessageTime = millis();
   Connected = true;
   Count ++;
  }
  
  if (contentMessage.startsWith("LIGHT:")) {
  String valueString = contentMessage.substring(6); // Extract the value from the message
  LightValue = Integer.parseInt(valueString);
  }
   
  if (contentMessage.startsWith("TEMPERATURE:")) {
  TemperatureString  = contentMessage.substring(12); // Extract the value from the message
  TemperatureValue =float(TemperatureString);
  TempValue = int(TemperatureString);
  }
 if(Count ==20){
   Count = 0;
   myPort.clear();
